...
namespace eudaq {
   ...
   namespace {
      auto dummy0 = Factory<RunControl>::Register<RunControl, const std::string&>(RunControl::m_id_factory);
      auto dummy1 = Factory<RunControl>::Register<RunControl, const std::string&>(eudaq::cstr2hash("RunControl"));
   }
      ...
      for (auto &conn : conn_to_run) {
         if (conn->GetType() != "Producer" && conn->GetType() != "DataCollector") {
            ...
            while (1) {
               ...
               if (st && st->GetState() == Status::STATE_RUNNING) break;
               if (n > 30)
  ...
  void RunControl::StopRun(){
	...
	//TODO:: EUDAQ_THROW
      ...
      for (int n = 0; n <= 60; n++) {
         bool somebody_running = false;
         for (auto &conn : conn_to_stop) {
            if (conn->GetType() == "Producer") {
               auto st = GetConnectionStatus(conn);
               if (st && st->GetState() == Status::STATE_RUNNING) {
                  somebody_running = true;
                  std::cout << "Producer " << conn->GetName() << " still running. Waiting." << std::endl;
                  break; //one running producer is enough to start the waiting
               }
            }
         }
         if (somebody_running) {
      ...
      //wait for everything to be stopped before exiting the stoprun method
      for (int n = 0; n <= 600; n++) {
         bool somebody_running = false;
         for (auto &conn : conn_to_stop) {
            auto st = GetConnectionStatus(conn);
         ...
         if (somebody_running) std::this_thread::sleep_for(std::chrono::milliseconds(1000));
         if (n == 600) {
            EUDAQ_ERROR("Timesout waiting running status");
         }
      }
  ...
  void RunControl::CommandHandler(TransportEvent &ev){
      ...
      switch (ev.etype) {
         case (TransportEvent::CONNECT):
            if (m_listening) {
               ...
               EUDAQ_INFO(std::string("Refused connect:    ") + con->GetName());
               m_cmdserver->SendPacket("ERROR EUDAQ CMD Not accepting new connections", *con, true);
               m_cmdserver->Close(*con);
               m_conn_status.erase(con);
            }
            break;
   ...
   void RunControl::CloseRunControl() {
      m_exit = true;
      if (m_thd_status.joinable()) m_thd_status.join();
      if (m_thd_server.joinable()) m_thd_server.join();
   }
