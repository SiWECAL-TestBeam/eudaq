...
//newLED
	...
	void ScReader::Read(std::deque<unsigned char> &buf, std::deque<eudaq::EventUP> &deqEvent) {
		...
		static const unsigned char C_PKTHDR_SPIROCDATA[4] = { 0x41, 0x43, 0x48, 0x41 };
						...
						if ((unsigned char) buf[1] == magic_hvadj[1]) {
							...
							EUDAQ_EXTRA(
									"HV_adjust: LDA=" + to_string((unsigned int )buf[2]) + " port=" + to_string((unsigned int )buf[3]) + " module="
											+ to_string((unsigned int ) buf[4]) + " HV1=" + to_string((unsigned int )buf[6]) + " HV2="
											+ to_string((unsigned int )buf[7])
...
//               std::cout << "DEBUG   mk3b getting more data" << std::endl << std::flush;
				...
				bool TimestampFlag = (statusHi == 0x08 && buf[10] == 0x45 && buf[11] == 0x4D && buf[12] == 0x49 && buf[13] == 0x54);
...
//               std::cout << "DEBUG: Analyzing Timestamp, ROC " << LDA_Header_cycle << std::endl;
				...
				if ((length == 0x08) && (statusHi == 0x10) && ((unsigned char) buf[10] == 0x02) && ((unsigned char) buf[11] == 0x83)
				...
				deque<unsigned char>::iterator dataPart = buf.begin() + e_sizeLdaHeader;

				//treating the KLauS packets.
				// cd cd 16 00 00 00 0a 00 01 80 41 43 94 00 49 41 07 00 00 01 94 e0 f0 d1 dd fe dd dd ee dd ab ab
				// cd cd 12 00 00 00 0a 00 01 80 41 43 01 00 7a 00 05 00 02 64 01 e0 f0 d1 40 00 ab ab
				if (statusHi & 0x80) { //seventh bit for the status == readout packet
					if ((statusHi & (0x20 | 0x10 | 0x40)) == 0) {
						//packet is not merged (0x20), not a config packet (0x10) and not flagged as ASIC readout (0x40)
						if ((statusLo & 0x01) == 0x00) {
							std::cout << "ERROR: unexpected status bits in status word: << 0x" << to_hex(statusHi, 2) << to_hex(statusLo, 2) << std::endl;
						}
				...
				if (!(statusHi & 0x40)) { // (not a readout packet)  //cd cd 0c 00 54 00 0b 08 02 20 02 cc 0f 0f 0e 00 03 00 00 00 00 00
					//We'll drop non-ASIC data packet;
					if ((length == 12) && (statusHi = 0x00) && (dataPart[0] == 0x02) && (dataPart[1] = 0xcc)) {
						//klaus dif without merging: end of readout
						buf.erase(buf.begin(), buf.begin() + length + e_sizeLdaHeader);
						continue;
					}
					if ((length == 12) && (statusHi = 0x20) && (dataPart[0] == 0x02) && (dataPart[1] = 0xcc)) {
						//SPIROC style end of readout
						buf.erase(buf.begin(), buf.begin() + length + e_sizeLdaHeader);
						continue;
					}
					if (_producer->getColoredTerminalMessages()) std::cout << "\033[31;1m";
					std::cout << "ERROR: unexpected packet type 0x" << to_hex(statusHi) << ", erasing " << length << " and " << e_sizeLdaHeader << endl;
...
//      std::cout << "DEBUG: buildEvents size _LDAAsicData=" << _LDAAsicData.size() << " _LDAKLAUSAsicData=" << _LDAKLAUSAsicData.size() << " _LDATimestampData=" << _LDATimestampData.size() << std::endl << std::flush;
	...
	eudaq::EventUP ScReader::insertMissedTrigger(const int roc, const uint64_t startTS, const int lastBuiltEventNr, const int ErrorStatus) {
		...
		EUDAQ_ERROR_STREAMOUT("Missed record of trigger " + to_string(lastBuiltEventNr) + " in Run=" + to_string(_runNo) + "). ROC=" + to_string(roc),
				std::cout,
				std::cerr);
	...
	eudaq::EventUP ScReader::insertEmptyEvent(const uint64_t stopTS, const uint64_t startTS, const int RawTriggerID, const uint64_t triggerTs,
			const int lastBuiltEventNr, const int triggerBxid, const int roc, int &ErrorStatus) {
...
//         if (_LDATimestampData.begin()==_LDATimestampData.end)
			...
			if (std::abs(firstTsROC - firstAsicROC) > 4) { //happens, that there is not even a noise hit for a long time
				colorPrint(TERMCOLOR_RED,
						"WARNING EB: first readout cycle in TS and ASIC queues too far apart: TS_ROC=" + to_string(_LDATimestampData.begin()->first)
								+ " ASIC_ROC="
...
//                  std::cout << "Trigger info BXID=" << bxid << "\tTrigID=" << triggerId << std::endl;
						if (bxid > 65535) {
							std::cout << "\033[34mWARNING EB: calculated trigger bxid too high: " << bxid << " in ROC " << processedROC << "\033[0m"
									<< std::endl;
							continue;
						}
...
//               std::cout << "DEBUG: insertEmptyEvent due to ahcal bxid=" << AhcalBxid << " trigbxid=" << triggerBxid << std::endl;
					if (_producer->getMinEventHits() == 0) {
						EventQueue.push_back(
								std::move(
										insertEmptyEvent(stopTS, startTS, triggerRawTrigId, triggerTs, ++_lastBuiltEventNr, triggerBxid, processedROC,
												ErrorStatus)));
//               trigIt++;
			...
			if (_LDATimestampData.count(roc)) {
				...
				for (int i = 0; i < _LDATimestampData[roc].TS_Triggers.size(); ++i) {
					if (_LDATimestampData[roc].TS_Triggers[i] < _LDATimestampData[roc].TS_Start) {
						std::cout << "ERROR EB: Trigger timestamp before the AHCAL started measuring. TrigID:" << _LDATimestampData[roc].TriggerIDs[i]
								<< std::endl;
						continue;
					}
					...
					while ((_producer->getInsertDummyPackets())
							&& (++_lastBuiltEventNr < (_LDATimestampData[roc].TriggerIDs[i] + _producer->getLdaTrigidOffset()))) {
...
//         std::cout << "#>>DEBUG idx=" << bxididx << "\tbxid=" << bxid << "\tcell=" << memCell << "\tfilled=" << MemoryCellsFilled << std::endl;
			...
			if (bxid > _producer->getMaximumBxid()) {
				...
				EUDAQ_WARN(
						"Throwing away BXID above threshold: bxid=" + to_string(bxid) + " ROC=" + to_string(_cycleNo) + " port=" + to_string(LDA_Header_port));
			}
...
//                  << " modulo 256), received=" << (int) LDA_Header_cycle << endl;
				EUDAQ_EXTRA(
						"Received a timestamp from previus ROC in run " + to_string(_runNo) + ". Global ROC=" + to_string(_cycleNoTS) + " ("
								+ to_string(_cycleNoTS % 256) + " modulo 256), received=" + to_string((int )LDA_Header_cycle) + " Type"
								+ to_string((int )TStype));
				//            LDA_cycle--;
			}
//
...
//         }
			...
			if ((cycle_difference > _producer->getMaxRocJump()) || (cycle_difference < (0 - _producer->getMaxRocJump()))) {
				...
				if (_producer->getColoredTerminalMessages()) std::cout << "\033[0m";
				EUDAQ_ERROR(
						"ERROR: Jump in run " + to_string(_runNo) + "in TS readoutcycle by " + to_string((int )cycle_difference) + "in ROC "
								+ to_string(LDA_cycle));
			...
			if (TStype == C_TSTYPE_TRIGID_INC) { //TODO
				...
				if (trigIDdifference != 1) { //serious error, we missed a trigger ID, we got it more time, or the data is corrupted
					...
					if ((trigIDdifference < 1) || (trigIDdifference >= _producer->getMaxTrigidSkip())) {
						...
						if (_producer->getColoredTerminalMessages()) std::cout << "\033[0m";
						EUDAQ_ERROR(
								"Unexpected TriggerID in run " + to_string(_runNo) + ". ROC=" + to_string(_cycleNoTS) + ", Expected TrigID="
										+ to_string(_trigID + 1)
...
*/
	...
	void ScReader::RunTimeStatistics::append(const RunTimeStatistics &otherStats) {
		...
		for (std::map<int, int>::const_iterator it = otherStats.triggers_in_cycle_histogram.cbegin(); it != otherStats.triggers_in_cycle_histogram.end();
				++it) {
