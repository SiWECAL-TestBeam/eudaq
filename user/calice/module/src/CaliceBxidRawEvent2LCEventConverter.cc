...
namespace eudaq {
   ...
   class CaliceBxidRawEvent2LCEventConverter: public LCEventConverter {
      public:
         bool Converting(EventSPC d1, LCEventSP d2, ConfigurationSPC conf) const override;
         static const uint32_t m_id_factory = cstr2hash("CaliceBxid");
      private:
   ...
   namespace {
      auto dummy0 = Factory<LCEventConverter>::Register<CaliceBxidRawEvent2LCEventConverter>(CaliceBxidRawEvent2LCEventConverter::m_id_factory);
   }

   static const char* EVENT_TYPE = "CaliceBxid";
   ...
   bool CaliceBxidRawEvent2LCEventConverter::Converting(EventSPC d1, LCEventSP d2, ConfigurationSPC conf) const {
      // try to cast the Event

     auto& fullevent = *(d1.get());
     int nsubevents=fullevent.GetNumSubEvent();
    for(int isub=0; isub<nsubevents; isub++) {

      std::cout<<nsubevents<<" "<<isub<<std::endl;
       auto& source=*(fullevent.GetSubEvent(isub));
...
//            std::cout << "DEBUG shiftedUnixTS=" << shiftedUnixTS << std::endl;
   ...
   LCCollectionVec* CaliceBxidRawEvent2LCEventConverter::createCollectionVec(lcio::LCEvent &result, string colName, string dataDesc, time_t timestamp,
   ...
   void CaliceBxidRawEvent2LCEventConverter::getScCALTemperatureSubEvent(const std::vector<uint8_t>& bl, LCCollectionVec *col) const {
   ...
   void CaliceBxidRawEvent2LCEventConverter::getScCALHVSubEvent(const std::vector<uint8_t>& bl, LCCollectionVec *col) const {
   ...
   void CaliceBxidRawEvent2LCEventConverter::getDataLCIOGenericObject(const std::vector<uint8_t> & bl, LCCollectionVec *col, int nblock) const {
   ...
   void CaliceBxidRawEvent2LCEventConverter::getDataLCIOGenericObject(eudaq::RawDataEvent const * rawev, LCCollectionVec *col, int nblock, int nblock_max) const {
